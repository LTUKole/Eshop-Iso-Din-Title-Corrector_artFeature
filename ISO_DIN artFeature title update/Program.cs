...
public class Program
{
    ...
    //  Version: 1.6 (External Configuration)
    ...
    public static async Task Main(string[] args)
    {
        ...
        // Validate that the connection string was loaded
        if (string.IsNullOrEmpty(connectionString))
        {
            Log.Fatal("Database connection string is not configured in appsettings.json.");
            Console.WriteLine("FATAL ERROR: Connection string 'DefaultConnection' not found in appsettings.json.");
            Log.CloseAndFlush();
            Console.ReadKey();
            return;
        }
